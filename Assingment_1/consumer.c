...
int main()
{
    const char *name = "OS_HW1_AndrewPeklar"; // Name of shared memory block to be passed to shm_open
    ...
    printf("Consumer reading: In  idx = %d\n", in-1);
    printf("Consumer reading: out idx = %d\n", out);
    ...
    for(i = 0; i < itemCnt; i++) {
        out = GetOut();
        while(GetIn() == out); //do nothin if equal
        val = ReadAtBufIndex(out);
        //i+1 for item count starting at 1; index vals start at 0
        printf("Consuming Item %4d with value %3d at Index %d\n", i+1, val, out);
